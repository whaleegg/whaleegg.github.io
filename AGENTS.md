...
`tags`는 소문자로 유지합니다. `categories`는 보통 1~2단계 이내로 제한합니다. `toc: false`, `comments: false`, `pin: true`, `math: true` 같은 옵션은 필요한 글에서만 사용합니다. 이미지는 `assets/img/...` 아래에 두고 root-relative path로 참조합니다. 가능하면 `width`와 `height`도 함께 지정해 레이아웃 흔들림을 줄입니다.
